/**
 * Syrian Home POS - Store, Printing, Payment Fees & Data Management Controller (v2.5.4)
...
class SettingsController {
  ...
  async triggerSystemReset(mode) {
    ...
    if (mode === 'factory_reset_all') {
      ...
      if (trimmed !== 'تأكيد' && trimmed !== 'تاكيد' && trimmed !== 'نعم' && trimmed !== 'confirm') {
        window.app?.showToast('لم يتم كتابة كلمة "تأكيد" بشكل صحيح، تم إلغاء العملية للأمان', 'warning');
        return;
      }
    } else {
    ...
    try {
      ...
      if (res && res.success) {
        ...
        window.app?.showToast(res?.error || 'فشلت العملية. يرجى التأكد من الاتصال بالسيرفر.', 'error');
      }
    } catch (err) {
      window.app?.showLoading(false);
      console.error('System reset error:', err);
      window.app?.showToast('خطأ أثناء تنفيذ العملية: ' + (err.message || err), 'error');
    }
  }
}
