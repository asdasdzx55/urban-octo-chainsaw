      ...
      return `
        <div class="bg-white dark:bg-gray-800 rounded-3xl p-5 border border-gray-200/80 dark:border-gray-700 shadow-xs hover:shadow-md transition flex flex-col justify-between gap-4 relative overflow-hidden group">
          ...
          <div class="grid grid-cols-3 gap-1.5 pt-1">
            <button onclick="window.employeesController.openSalaryModal(${emp.id})" class="py-2 px-1 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[11px] flex items-center justify-center gap-1 shadow-2xs transition cursor-pointer" title="دفع الراتب الشهري">
              <i data-lucide="banknote" class="w-3.5 h-3.5"></i>
              <span>دفع راتب</span>
            </button>

            <button onclick="window.employeesController.openAdvanceModal(${emp.id})" class="py-2 px-1 rounded-xl bg-amber-600 hover:bg-amber-700 text-white font-bold text-[11px] flex items-center justify-center gap-1 shadow-2xs transition cursor-pointer" title="صرف سلفة">
              <i data-lucide="hand-coins" class="w-3.5 h-3.5"></i>
              <span>صرف سلفة</span>
            </button>

            <button onclick="window.employeesController.openLedgerModal(${emp.id})" class="py-2 px-1 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 font-bold text-[11px] flex items-center justify-center gap-1 border border-indigo-200 dark:border-indigo-800 transition cursor-pointer" title="كشف حساب العامل">
